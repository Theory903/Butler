        ...
        if self._client is None:
            try:
                ...
                if "butler_memories" in collection_names:
                    ...
                    current_dim = (
                        c_info.config.params.vectors.size
                        if hasattr(c_info.config.params.vectors, "size")
                        else 0
                    )
                    if current_dim != 1536:
                        logger.warning(
                            f"Qdrant collection dimension mismatch: {current_dim} vs 1536. Recreating..."
                        )
                ...
                logger.info(
                    f"Qdrant connection established at {settings.QDRANT_HOST}:{settings.QDRANT_PORT}"
                )
