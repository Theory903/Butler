...
from domain.meetings.models import Meeting, Transcription, MeetingSummary
...
class MeetingService:
    ...
    async def create_meeting(self, account_id: str, title: str, participants: List[str] = []) -> Meeting:
        meeting = Meeting(
    ...
    async def add_transcription_segment(self, meeting_id: str, speaker: str, text: str, timestamp: float) -> None:
    ...
    async def get_meeting_history(self, account_id: str) -> List[Meeting]:
        stmt = select(Meeting).where(Meeting.account_id == uuid.UUID(account_id)).order_by(Meeting.created_at.desc())
        result = await self._db.execute(stmt)
