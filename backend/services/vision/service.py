...
class VisionService:
    ...
    def __init__(
        self, proxy: VisionModelProxy | None = None, processor: MediaProcessor | None = None
    ):
        self.proxy = proxy or VisionModelProxy()
        self._processor = processor or MediaProcessor()

    async def detect(
        self, image_data: bytes, classes: list[str] = None, threshold: float = 0.5
    ) -> dict:
    ...
    async def process_video(
        self, video_data: bytes, task: str = "Describe what is happening."
    ) -> dict:
        ...
        return {
            ...
            "metadata": {"frames_processed": len(frames), "model": "qwen2.5-vl-7b-segmented"},
        }
