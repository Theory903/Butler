...
from domain.skills.manifest import SkillManifest, RiskTier
...
class TrustGateResult:
    def __init__(self, gate: str, success: bool, details: Optional[Dict[str, Any]] = None, error: Optional[str] = None):
        self.gate = gate
...
class TrustPipeline:
    ...
    def __init__(self, semgrep_rules_path: Optional[str] = None):
        self.semgrep_rules_path = semgrep_rules_path or getattr(settings, "SEMGREP_RULES_PATH", None)

    async def verify_all(self, manifest_data: Dict[str, Any], archive_path: Path, signature: Optional[str] = None) -> List[TrustGateResult]:
        ...
        if not gate_a.success: return results
        
        # Gate B: Schema
        gate_b = self.verify_gate_b(manifest_data)
        results.append(gate_b)
        if not gate_b.success: return results
        ...
        if not gate_c.success: return results
    ...
    def verify_gate_a(self, archive_path: Path, signature: Optional[str], public_key_hex: Optional[str]) -> TrustGateResult:
        """Gate A: ED25519 Signature Verification."""
        if not signature or not public_key_hex:
            return TrustGateResult("Gate A: Signature", False, error="Missing signature or public key")
        ...
        try:
            ...
            return TrustGateResult("Gate A: Signature", False, error=f"Signature verification error: {str(e)}")
    ...
    def verify_gate_b(self, manifest_data: Dict[str, Any]) -> TrustGateResult:
        ...
        try:
            ...
            return TrustGateResult("Gate B: Schema", False, error=f"Manifest schema violation: {str(e)}")
    ...
    async def verify_gate_c(self, archive_path: Path) -> TrustGateResult:
        ...
        dangerous_patterns = [
            b"import os", b"import subprocess", b"eval(", b"exec(", b"getattr(", b"setattr("
        ]
        ...
        try:
            ...
            return TrustGateResult("Gate C: Static Analysis", False, error=f"Static analysis failed: {str(e)}")
