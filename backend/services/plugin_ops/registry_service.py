    ...
    async def install_package(self, package_id: str, version: str, actor_id: uuid.UUID) -> bool:
        ...
        try:
            ...
            audit = SecurityAuditLog(
                ...
                gate_results={r.gate: {"success": r.success, "details": r.details, "error": r.error} for r in gate_results},
            ...
            if not all_passed:
                ...
                logger.error("install_failed_security_gates", package_id=package_id, failed=failed_gates)
                return False

            # 4. Filesystem Staging
            manifest = SkillManifest(**manifest_data)
            storage_path = await self.lifecycle.stage_version(package_id, version, archive_bytes, manifest)
