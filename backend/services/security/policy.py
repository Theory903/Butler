...
class PolicyDecisionPoint:
    ...
    async def evaluate(self, input: PolicyInput) -> PolicyDecision:
        ...
        if input.action.startswith("communication:") and input.approval_state != "approved":
            return PolicyDecision(
                allow=False,
    ...
    async def validate(self, request: ToolGateRequest, actor: ActorContext) -> ToolGateDecision:
        ...
        if capability.approval_class == "explicit" and not request.approval_token:
            return ToolGateDecision(
                allowed=False, reason="Requires approval", requires_approval=True
            )
...
class MemoryIsolation:
    ...
    async def check_retrieval(
        self, memory_class: str, task_family: str, assurance: str
    ) -> RetrievalDecision:
