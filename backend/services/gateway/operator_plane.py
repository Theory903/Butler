...
BREAK_GLASS_ACTIVE = Gauge(
    "butler_break_glass_active",
    "Break glass mode active status"
)


class OperatorScope(str, enum.Enum):
    ...
    def __init__(self):
        self._allowed_actions: Dict[OperatorScope, Set[str]] = {
            OperatorScope.PERSONAL: {
                "user:read", "user:update", "session:list", "session:terminate"
            },
            OperatorScope.TENANT: {
                "tenant:read", "tenant:update", "user:list", "user:invite",
                "user:suspend", "audit:read", "metrics:read"
            },
            OperatorScope.DOCTOR: {
                "system:diagnose", "health:read", "metrics:read", "logs:read",
            ...
            async def wrapper(identity: OperatorIdentity, *args, **kwargs) -> Any:
                with tracer.start_as_current_span(f"rbac.enforce.{action}") as span:
                    ...
                    if not identity.scope.includes(required_scope):
                        ...
                        OPERATOR_ACTIONS.add(1, {"scope": identity.scope, "action": action, "result": "denied"})
                        raise PermissionError(f"Insufficient scope: {identity.scope} < {required_scope}")
                    ...
                    if not self.is_allowed(identity, action):
                        ...
                        OPERATOR_ACTIONS.add(1, {"scope": identity.scope, "action": action, "result": "denied"})
                        raise PermissionError(f"Action not allowed: {action} for scope {identity.scope}")

                    # Rate limit check
                    if not self._rate_limiters[identity.scope].try_acquire(str(identity.operator_id)):
                        span.set_status(Status(StatusCode.ERROR, "rate_limited"))
                        OPERATOR_ACTIONS.add(1, {"scope": identity.scope, "action": action, "result": "rate_limited"})
                    ...
                    if circuit_breaker.state == CircuitBreakerState.OPEN:
                        ...
                        OPERATOR_ACTIONS.add(1, {"scope": identity.scope, "action": action, "result": "circuit_open"})
                        raise RuntimeError("Operation temporarily unavailable due to high failure rate")
                    ...
                    try:
                        ...
                        OPERATOR_ACTIONS.add(1, {"scope": identity.scope, "action": action, "result": "success"})
                        ...
                        OPERATOR_ACTIONS.add(1, {"scope": identity.scope, "action": action, "result": "failed"})
                        span.set_status(Status(StatusCode.ERROR, str(e)))
                        raise
                    finally:
                        duration = time.perf_counter() - start_time
                        OPERATOR_LATENCY.record(duration, {"scope": identity.scope, "action": action})
...
class Doctor:
    ...
    async def get_metrics(self, identity: OperatorIdentity, metric_names: List[str]) -> Dict[str, Any]:
...
class BreakGlass:
    ...
    def activate(self, identity: OperatorIdentity, justification: str, duration: int = 300) -> uuid.UUID:
        """
