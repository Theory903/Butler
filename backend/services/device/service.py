...
class DeviceService:
    ...
    def __init__(self, redis: Redis, db: AsyncSession, tenant_id: str | None = None):
        self._redis = redis
        self._db = db
        self._tenant_id = tenant_id
        self._adapters = AdapterRegistry()

    def _device_state_key(self, device_id: str) -> str:
        """Generate tenant-scoped device state key."""
        if self._tenant_id:
            namespace = get_tenant_namespace(self._tenant_id)
            return f"{namespace.prefix}:device:{device_id}:state"
    ...
    async def list_devices(self, account_id: str) -> list[DeviceRegistry]:
        stmt = select(DeviceRegistry).where(
            DeviceRegistry.owner_account_id == uuid.UUID(account_id)
        )
    ...
    async def get_state(self, device_id: str) -> dict[str, Any]:
        ...
        """
        cached = await self._redis.get(self._device_state_key(device_id))
        if cached:
    ...
    async def dispatch_command(
        self, requester_account_id: str, device_id: str, action: str, params: dict
    ) -> dict[str, Any]:
        """
        ...
        adapter = self._adapters.resolve(device.protocol)
        logger.info(
            "device_command_dispatching", device=device_id, protocol=device.protocol, action=action
        )
        ...
        try:
            ...
            await self._redis.delete(self._device_state_key(device_id))
            return result
        except Exception as e:
