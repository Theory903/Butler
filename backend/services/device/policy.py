...
class DevicePolicy:
    ...
    def evaluate_dispatch(device: DeviceRegistry, requester_account_id: str) -> PolicyResult:
        ...
        if str(device.owner_account_id) != requester_account_id:
            logger.warning(
                "policy_denied_ownership",
                expected=str(device.owner_account_id),
                received=requester_account_id,
            )
        ...
        if device.online_state == "offline":
            ...
            return PolicyResult(
                allowed=False,
                reason="Device is explicitly flagged as unreachable or offline natively.",
            )
        ...
        if device.trust_state != "trusted":
            ...
            return PolicyResult(
                allowed=False,
                reason=f"Device pairing state is '{device.trust_state}'. Explicit trust required.",
            )
