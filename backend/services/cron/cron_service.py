...
from domain.cron.models import ButlerCronJob, ButlerCronRun, CronDeliveryMode, CronJobStatus
    ...
    async def create_job(
        ...
        active_count_result = await db.execute(
            select(ButlerCronJob).where(
                ButlerCronJob.account_id == account_id, ButlerCronJob.status == CronJobStatus.ACTIVE
            )
        )
        if len(active_count_result.scalars().all()) >= _MAX_ACTIVE_JOBS_PER_ACCOUNT:
            raise ValueError(
                f"Max active jobs ({_MAX_ACTIVE_JOBS_PER_ACCOUNT}) reached for account."
            )
