...
class VoyageEmbeddingProvider:
    ...
    def __init__(
        ...
        self._safe_client = SafeRequestClient(timeout=_DEFAULT_TIMEOUT) if tenant_id else None
        # Fallback to direct httpx for non-tenant contexts (e.g., system-level embedding)
    ...
    async def embed(self, text: str) -> EmbeddingResult:
        ...
        if self._safe_client and self._tenant_id:
            response = await self._safe_client.post(
                url, tenant_id=self._tenant_id, json=payload, headers=headers
            )
        else:
    ...
    async def embed_batch(self, texts: list[str]) -> list[EmbeddingResult]:
        ...
        for item in data["data"]:
            results.append(EmbeddingResult(embedding=item["embedding"], model=self._model))
...
class CohereEmbeddingProvider:
    ...
    async def embed_batch(self, texts: list[str]) -> list[EmbeddingResult]:
        ...
        for embedding in data["embeddings"]:
            results.append(EmbeddingResult(embedding=embedding, model=self._model))
