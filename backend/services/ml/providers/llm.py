...
class GoogleGeminiNativeProvider(BaseReasoningHTTPProvider):
    provider_name = "google"
    ...
    def __init__(
        ...
        try:
            ...
            raise ImportError("google-genai is required for GoogleGeminiNativeProvider. Install it with: pip install google-genai")
    ...
    async def generate(self, request: ReasoningRequest) -> ReasoningResponse:
        ...
        contents = [
            self._types.Content(
                role="user",
                parts=[self._types.Part.from_text(text=request.prompt)],
            )
        ]

        if request.system_prompt:
            contents.insert(0, self._types.Content(
                role="system",
                parts=[self._types.Part.from_text(text=request.system_prompt)],
            ))
        ...
        if request.tools:
            for tool in request.tools:
                if isinstance(tool, dict) and tool.get("type") == "function":
                    # Convert OpenAI function format to Gemini function declaration
                    func_def = tool["function"]
                    tools.append(self._types.Tool(
                        function_declarations=[self._types.FunctionDeclaration(
                            name=func_def["name"],
                            description=func_def.get("description", ""),
                            parameters=self._types.Schema(
                                type=self._types.Type.OBJECT,
                                properties={
                                    k: self._types.Schema(
                                        type=self._types.Type.STRING,
                                    )
                                    for k in func_def.get("parameters", {}).get("properties", {}).keys()
                                },
