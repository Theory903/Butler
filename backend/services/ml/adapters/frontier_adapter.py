...
class FrontierAdapter:
    ...
    All external API calls are protected by SSRF egress policy.
    """
    ...
    async def _execute_gemini(
        self, model: str, prompt: str, params: dict[str, Any], start: float
    ) -> dict[str, Any]:
        ...
        decision, reason = self._egress_policy.check_url(
            "https://generativelanguage.googleapis.com", self.tenant_id
        )
        if decision == EgressDecision.DENY:
            raise RuntimeError(f"Egress policy denied Gemini API call: {reason}")
    ...
    async def _execute_openai(
        ...
        # SSRF protection check
        decision, reason = self._egress_policy.check_url("https://api.openai.com", self.tenant_id)
        if decision == EgressDecision.DENY:
