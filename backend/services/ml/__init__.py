from .admin import MLAdmin
from .embeddings import EmbeddingService
from .intent import IntentClassifier
from .mixer import CandidateMixer, SignalManager
from .ranking import HeavyRanker, LightRanker
from .registry import ModelRegistry
from .runtime import MLRuntimeManager


class MLService:
    """Butler Intelligence Platform (v3.0 RIO) Facade with multi-tenant support.
    ...
    async def classify_intent(self, text: str, tenant_id: str | None = None):
        return await self._classifier.classify(text)

    async def embed(self, text: str, tenant_id: str | None = None):
        return await self._embedder.embed(text)

    async def mix_and_rank(
        self, query: str, entity_id: str, limit: int = 10, tenant_id: str | None = None
    ):
        """Unified Twitter-style pipeline: Mix -> Enrich -> Rank with tenant isolation."""
        if not self._mixer:
            return []
    ...
    async def rerank(
        self, query: str, candidates: list, user_id: str = None, tenant_id: str | None = None
    ):
    ...
    async def get_features(self, entity_id: str, feature_names: list, tenant_id: str | None = None):
