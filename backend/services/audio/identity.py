...
class VoiceIdentityManager:
    ...
    async def identify_speaker(self, audio_data: bytes) -> str | None:
        """
        ...
        logger.debug(
            "speaker_identification_attempt",
            similarity=max_similarity,
            matched=max_similarity > self.threshold,
        )
        ...
        try:
            ...
            async with async_session_factory() as db:
                ...
                if profile:
                    ...
                    profile = VoiceProfile(account_id=account_id, embedding=embedding)
                    db.add(profile)
                await db.commit()
