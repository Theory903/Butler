...
class SpeakerDiarization:
        ...
        if self._pipeline is None:
            ...
            try:
                ...
                self._pipeline = Pipeline.from_pretrained(
                    settings.DIARIZATION_MODEL, use_auth_token=self.hf_token
                )
    ...
    async def diarize(
        self, audio_data: bytes, min_speakers: int = 1, max_speakers: int = 10
    ) -> DiarizationResult:
        """
        ...
        if pipeline is None:
            ...
            return DiarizationResult(
                segments=[
                    SpeakerSegment(speaker_id="SPEAKER_00", start=0.0, end=1.0, confidence=1.0)
                ],
                speaker_count=1,
