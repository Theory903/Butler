...
class PostgresNotebookRepository(INotebookRepository):
    ...
    async def create_notebook(self, account_id: str, name: str, description: str = "") -> Notebook:
        notebook = Notebook(account_id=uuid.UUID(account_id), name=name, description=description)
    ...
    async def get_notebooks(self, account_id: str, archived: bool = False) -> list[Notebook]:
        stmt = (
            select(Notebook)
            .where(Notebook.account_id == uuid.UUID(account_id), Notebook.archived == archived)
    ...
    async def get_notebook(self, account_id: str, notebook_id: str) -> Notebook | None:
        stmt = (
            select(Notebook)
            .where(
                Notebook.account_id == uuid.UUID(account_id), Notebook.id == uuid.UUID(notebook_id)
            )
    ...
    async def add_source(
        ...
        # 2. Create source
        source = Source(
            account_id=notebook.account_id, title=title, source_type=source_type, asset=asset
        )
        self._db.add(source)
        await self._db.flush()
    ...
    async def get_sources(self, notebook_id: str) -> list[Source]:
        stmt = (
            select(Source)
            .join(NotebookSources)
            .where(NotebookSources.notebook_id == uuid.UUID(notebook_id))
    ...
    async def add_note(
        ...
        note = Note(
            account_id=notebook.account_id, title=title, content=content, note_type=note_type
        )
        self._db.add(note)
        await self._db.flush()
    ...
    async def get_notes(self, notebook_id: str) -> list[Note]:
        stmt = (
            select(Note)
            .join(NotebookNotes)
            .where(NotebookNotes.notebook_id == uuid.UUID(notebook_id))
    ...
    async def get_shared_sources(self, source_id: str) -> list[Notebook]:
        ...
        stmt = (
            select(Notebook)
            .join(NotebookSources)
            .where(NotebookSources.source_id == uuid.UUID(source_id))
        )
        result = await self._db.execute(stmt)
        return list(result.scalars().all())

    async def search_notebook_content(
        self, notebook_id: str, query_vector: list[float], limit: int = 10
    ) -> list[Any]:
        """Hybrid search within a notebook using pgvector."""
        # This uses the 'source_embeddings' table linked to sources in this notebook
        stmt = (
            select(Source, SourceEmbedding)
            .join(SourceEmbedding)
            .join(NotebookSources, Source.id == NotebookSources.source_id)
            .where(NotebookSources.notebook_id == uuid.UUID(notebook_id))
            .order_by(SourceEmbedding.embedding.cosine_distance(query_vector))
            .limit(limit)
        )
    ...
    async def get_graph_breadth(self, start_notebook_id: str, depth: int = 2) -> list[dict]:
        ...
        result = await self._db.execute(
            sql, {"nb_id": uuid.UUID(start_notebook_id), "max_depth": depth}
        )
