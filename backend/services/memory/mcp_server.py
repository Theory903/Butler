...
async def mcp_query_memory(
    query_payload: dict[str, Any], account: AccountContext = Depends(get_current_user)
) -> dict[str, Any]:
    ...
    with tracer.start_as_current_span("mcp.memory.query"):
        ...
        try:
            ...
            entities = await knowledge_repo.search_entities(
                account_id=uuid.UUID(account.sub), query=query_text, limit=5
            )
            ...
            return {"jsonrpc": "2.0", "result": {"resources": resources}}
