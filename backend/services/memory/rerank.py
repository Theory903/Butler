...
class LocalReranker(Reranker):
    ...
    async def rerank(self, query: str, candidates: List[ScoredMemory]) -> List[ScoredMemory]:
        ...
        try:
            ...
            logger.debug(f"LocalReranker: Processing {len(rerank_candidates)} candidates using {self._model_name}")
...
class RerankCoordinator:
    ...
    def __init__(self, strategy: str = "local", api_key: Optional[str] = None):
