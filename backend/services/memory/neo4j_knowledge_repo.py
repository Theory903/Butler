...
class Neo4jKnowledgeRepo(KnowledgeRepoContract):
    ...
    async def upsert_entity(self, account_id: uuid.UUID, entity_type: str, name: str, 
                           summary: Optional[str] = None, metadata: Optional[Dict] = None) -> KnowledgeEntity:
        query = """
        MERGE (e:KnowledgeEntity {account_id: $account_id, name: $name, entity_type: $entity_type})
    ...
    async def archive_entity(self, account_id: uuid.UUID, entity_id: uuid.UUID, superseded_by: Optional[uuid.UUID] = None):
        """Standard archival logic for the Neo4j tier."""
        query = """
        MATCH (e:KnowledgeEntity {account_id: $account_id, id: $entity_id})
    ...
    async def store_chunk(self, account_id: uuid.UUID, chunk_id: uuid.UUID, text: str, 
                         source_id: uuid.UUID, source_type: str, entity_ids: List[uuid.UUID]):
        """Butler-native: Source Attribution. Link Chunks to Entities."""
        query = """
        MERGE (c:KnowledgeChunk {id: $chunk_id, account_id: $account_id})
        SET c.text = $text,
        ...
        MATCH (e:KnowledgeEntity {id: ent_id, account_id: $account_id})
    ...
    async def get_graph_context(self, account_id: uuid.UUID, entity_names: List[str], depth: int = 1) -> List[Dict]:
        """Graph-Vector Expansion: Find entities and their neighborhood context."""
        cypher = f"""
        MATCH (e:KnowledgeEntity {{account_id: $account_id}})
        WHERE e.name IN $entity_names AND e.status = 'active'
        MATCH (e)-[r*1..{depth}]-(related:KnowledgeEntity {{status: 'active'}})
        RETURN e.name AS source, type(r[0]) AS relation, related.name AS target, related.summary AS summary
        LIMIT 50
        """
        params = {"account_id": str(account_id), "entity_names": entity_names}
        return await self._client.execute_query(cypher, params)

    async def upsert_edge(self, account_id: uuid.UUID, source_id: uuid.UUID, target_id: uuid.UUID, 
        ...
        query = f"""
        MATCH (s:KnowledgeEntity {{account_id: $account_id, id: $source_id}})
        MATCH (t:KnowledgeEntity {{account_id: $account_id, id: $target_id}})
    ...
    async def search_entities(self, account_id: uuid.UUID, query: str, limit: int = 10) -> List[KnowledgeEntity]:
        # Using full-text search index if available in Neo4j
        cypher = """
        MATCH (e:KnowledgeEntity {account_id: $account_id, status: 'active'})
        WHERE e.name CONTAINS $query OR e.summary CONTAINS $query
        RETURN e LIMIT $limit
        """
        params = {"account_id": str(account_id), "query": query, "limit": limit}
        results = await self._client.execute_query(cypher, params)
        
        return [self._map_node_to_entity(r["e"]) for r in results]

    async def get_related_entities(self, account_id: uuid.UUID, entity_id: uuid.UUID, depth: int = 1) -> List[KnowledgeEntity]:
        cypher = f"""
        MATCH (e:KnowledgeEntity {{account_id: $account_id, id: $entity_id, status: 'active'}})-[*1..{depth}]-(related:KnowledgeEntity {{status: 'active'}})
        RETURN DISTINCT related LIMIT 20
        """
        params = {"account_id": str(account_id), "entity_id": str(entity_id)}
        results = await self._client.execute_query(cypher, params)
        
        return [self._map_node_to_entity(r["related"]) for r in results]

    async def resolve_identity(self, account_id: uuid.UUID, name: str) -> Optional[KnowledgeEntity]:
        cypher = """
        MATCH (e:KnowledgeEntity {account_id: $account_id, status: 'active'})
        WHERE e.name =~ $name_regex
        RETURN e LIMIT 1
        """
        params = {"account_id": str(account_id), "name_regex": f"(?i){name}"}
        results = await self._client.execute_query(cypher, params)
