...
class CommunicationPolicy:
    ...
    async def pre_send_check(self, request: SendRequest) -> PolicyResult:
        ...
        # 3. Sender Identity
        sender = await self._verify_sender(
            request.sender_profile_id, request.channel, request.priority_class
        )
    ...
    async def _check_consent(self, recipient: str, channel: str) -> PolicyResult:
        stmt = select(ConsentState).where(
            ConsentState.recipient == recipient, ConsentState.channel == channel
        )
    ...
    async def _verify_sender(
        self, profile_id: str, channel: str, priority_class: str
    ) -> PolicyResult:
    ...
    async def _verify_approval_token(self, token: str | None) -> PolicyResult:
