...
from api.schemas.communication import CanonicalInbound, AuthenticityResult
...
class WebhookValidator:
    ...
    async def verify_whatsapp(self, request: Request, body: bytes) -> bool:
        ...
        if not signature_header or not signature_header.startswith("sha256=") or not self.whatsapp_secret:
            return False
    ...
    async def verify_sendgrid(self, request: Request, body: bytes) -> bool:
        ...
        if not HAS_CRYPTOGRAPHY:
            logger.error("Rejecting SendGrid webhook: cryptography package missing. Cannot verify signature safely.")
            return False
...
class InboundNormalizer:
    ...
    async def normalize(self, provider: str, payload: Dict[str, Any]) -> Optional[CanonicalInbound]:
        if provider == "twilio":
