...
class ToolDefinition(Base):
    ...
    )  # search, communication, device, data
    risk_tier: Mapped[str] = mapped_column(
        String(32), nullable=False
    )  # T0_safe, T1_low, T2_medium, T3_high
    ...
    compensation_handler: Mapped[str] = mapped_column(
...
class ToolExecution(Base):
    ...
    __table_args__ = (Index("ix_tool_executions_tenant_created", "tenant_id", "created_at"),)
    ...
    task_id: Mapped[uuid.UUID] = mapped_column(PGUUID(as_uuid=True), nullable=True, index=True)
    workflow_id: Mapped[uuid.UUID] = mapped_column(PGUUID(as_uuid=True), nullable=True, index=True)
