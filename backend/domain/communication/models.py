...
from sqlalchemy import String, Boolean, JSON, DateTime, Integer, event
...
class DeliveryRecord(Base):
    ...
    first_provider_acceptance: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=True)
...
class ConsentState(Base):
    ...
    reason: Mapped[str] = mapped_column(String(255), nullable=True) # user_opt_out, hard_bounce, complaint
...
class SenderProfile(Base):
    ...
    type: Mapped[str] = mapped_column(String(50), nullable=False) # e.g., whatsapp_business, personal_email
    verified: Mapped[bool] = mapped_column(Boolean, default=False)
    
    capabilities: Mapped[List[str]] = mapped_column(JSON, default=list) # e.g., ["transactional", "marketing"]
