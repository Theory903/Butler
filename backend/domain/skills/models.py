...
from sqlalchemy import JSON, UUID, DateTime, Enum, ForeignKey, String, Text
...
class PluginPackage(Base):
    ...
    )  # e.g. "clawhub:anthropic-provider"
...
class PluginVersion(Base):
    ...
    package_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("butler_plugin_packages.id"), index=True
    )
    ...
    signature: Mapped[str | None] = mapped_column(Text)  # ED25519 signature
...
class SecurityAuditLog(Base):
    ...
    action: Mapped[str] = mapped_column(
        String(100)
    )  # "install", "promote", "rollback", "gate_failure"
    ...
    package: Mapped[PluginPackage | None] = relationship(
        "PluginPackage", back_populates="audit_logs"
    )
