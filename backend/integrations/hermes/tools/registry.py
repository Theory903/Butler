...
"""Central registry for all butler-agent tools.
...
def discover_builtin_tools(tools_dir: Optional[Path] = None) -> List[str]:
    ...
    module_names = [
        f"backend.integrations.hermes.tools.{path.stem}"
...
class ToolRegistry:
    ...
    def get_max_result_size(self, name: str, default: Optional[Union[int, float]] = None) -> Union[int, float]:
        ...
        if default is not None:
            return default
        from .budget_config import DEFAULT_RESULT_SIZE_CHARS
...
ButlerImportedToolRegistry = ToolRegistry
