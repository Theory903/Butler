...
def _get_file_ops(task_id: str = "default") -> ShellFileOperations:
    ...
    """
    import time
    terminal_runtime = cast(Any, importlib.import_module("backend.integrations.hermes.tools.terminal"))
    ...
    if cached is not None:
        with terminal_runtime._env_lock:
            if task_id in terminal_runtime._active_environments:
                terminal_runtime._last_activity[task_id] = time.time()
                return cached
            else:
    ...
    with terminal_runtime._creation_locks_lock:
        if task_id not in terminal_runtime._creation_locks:
            terminal_runtime._creation_locks[task_id] = threading.Lock()
        task_lock = terminal_runtime._creation_locks[task_id]
    ...
    with task_lock:
        ...
        with terminal_runtime._env_lock:
            if task_id in terminal_runtime._active_environments:
                terminal_runtime._last_activity[task_id] = time.time()
                terminal_env = terminal_runtime._active_environments[task_id]
            else:
                terminal_env = None

        if terminal_env is None:
            config = terminal_runtime._get_env_config()
            env_type = config["env_type"]
            overrides = terminal_runtime._task_env_overrides.get(task_id, {})
            ...
            terminal_env = terminal_runtime._create_environment(
                env_type=env_type,
                image=image,
                cwd=cwd,
            ...
            with terminal_runtime._env_lock:
                terminal_runtime._active_environments[task_id] = terminal_env
                terminal_runtime._last_activity[task_id] = time.time()

            terminal_runtime._start_cleanup_thread()
...
def _check_file_reqs():
    ...
    from . import check_file_requirements
...
def _handle_search_files(args, **kw):
    ...
    target = str(target_map.get(raw_target, raw_target or "content"))
    return search_tool(
