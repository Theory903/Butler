...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/butler-agent",
    ...
    lines.append(f"# Butler v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Butler. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Butler Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Butler Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Butler v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Butler v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Butler v{new_version} ({calver_date})' "
