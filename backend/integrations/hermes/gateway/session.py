...
class SessionStore:
    ...
    def append_to_transcript(self, session_id: str, message: Dict[str, Any], skip_db: bool = False) -> None:
        ...
        if self._db and not skip_db:
            try:
                db_content = message.get("content")
                if db_content is None:
                    db_content = ""
                elif not isinstance(db_content, str):
                    db_content = json.dumps(db_content, ensure_ascii=False)

                db_tool_name = message.get("tool_name")
                db_tool_call_id = message.get("tool_call_id")
        ...
        if self._db:
            try:
                ...
                for msg in messages:
                    ...
                    db_tool_name = msg.get("tool_name")
                    db_tool_call_id = msg.get("tool_call_id")
                    db_reasoning = msg.get("reasoning") if role == "assistant" else None
                    self._db.append_message(
                        session_id=session_id,
                        role=role,
                        content=db_content,
                        ...
                        tool_call_id=str(db_tool_call_id) if db_tool_call_id is not None else "",
                        reasoning=str(db_reasoning) if db_reasoning is not None else "",
