...
def is_network_accessible(host: str) -> bool:
    ...
    try:
        ...
        if mapped_addr is not None and mapped_addr.is_loopback:
            return False
        return True
    except ValueError:
...
async def cache_image_from_url(url: str, ext: str = ".jpg", retries: int = 2) -> str:
    ...
    if last_exc is not None:
        raise last_exc
    raise RuntimeError("image cache download failed without an exception")
...
async def cache_audio_from_url(url: str, ext: str = ".ogg", retries: int = 2) -> str:
    ...
    if last_exc is not None:
        raise last_exc
    raise RuntimeError("audio cache download failed without an exception")
...
class MessageEvent:
    ...
    # Source information
    source: SessionSource = field(default_factory=lambda: SessionSource(platform=Platform.LOCAL, chat_id=""))
...
class BasePlatformAdapter(ABC):
    ...
    def _acquire_platform_lock(self, scope: str, identity: str, resource_desc: str) -> bool:
        ...
        from ..status import acquire_scoped_lock
    ...
    def _release_platform_lock(self) -> None:
        ...
        if not identity:
            return
        from ..status import release_scoped_lock
