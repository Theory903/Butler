...
def _diff_ansi() -> dict[str, str]:
    ...
    try:
        skin_engine = importlib.import_module("backend.integrations.hermes.hermes_cli.skin_engine")
        get_active_skin = getattr(skin_engine, "get_active_skin")
...
def _resolve_skill_manage_paths(args: dict) -> list[Path]:
    ...
    if action == "create":
        ...
        skill_dir = _resolve_skill_dir(name, str(category) if category is not None else "")
...
class KawaiiSpinner:
    ...
    def _is_patch_stdout_proxy(self) -> bool:
        ...
        try:
            ...
            StdoutProxy = getattr(patch_stdout_module, "StdoutProxy")
