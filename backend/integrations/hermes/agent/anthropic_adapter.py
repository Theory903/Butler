...
try:
    _anthropic_sdk = importlib.import_module("anthropic")
except ImportError:
    _anthropic_sdk = None
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Butler", "Butler")
                text = text.replace("Butler", "Butler")
                text = text.replace("butler-agent", "butler")
                text = text.replace("Nous Research", "Butler")
                block["text"] = text
