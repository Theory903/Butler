"""ACP agent server — exposes Butler via the Agent Client Protocol."""
...
try:
    hermes_cli_module = importlib.import_module("backend.integrations.hermes.hermes_cli")
    HERMES_VERSION = getattr(hermes_cli_module, "__version__", "0.0.0")
except Exception:
...
class HermesACPAgent(acp.Agent):
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            disabled_toolsets = list(getattr(state.agent, "disabled_toolsets", None) or [])
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="butler-agent", version=HERMES_VERSION),
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        if approval_cb:
            try:
                terminal_module = cast(Any, importlib.import_module("backend.integrations.hermes.tools.terminal"))
                previous_approval_cb = getattr(terminal_module, "_approval_callback", None)
                terminal_module.set_approval_callback(approval_cb)
            except Exception:
        ...
        def _run_agent() -> dict:
            try:
                ...
                if approval_cb:
                    try:
                        terminal_module = cast(Any, importlib.import_module("backend.integrations.hermes.tools.terminal"))
                        terminal_module.set_approval_callback(previous_approval_cb)
                    except Exception:
    ...
    def _cmd_model(self, args: str, state: SessionState) -> str:
        ...
        try:
            models_module = importlib.import_module("backend.integrations.hermes.hermes_cli.models")
            parse_model_input = getattr(models_module, "parse_model_input")
            detect_provider_for_model = getattr(models_module, "detect_provider_for_model")
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Butler v{HERMES_VERSION}"
