...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    if not task_id:
        return
    try:
        from ..tools.terminal import register_task_env_overrides
...
def _clear_task_cwd(task_id: str) -> None:
    ...
    if not task_id:
        return
    try:
        from ..tools.terminal import clear_task_env_overrides
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Butler."""
...
class SessionManager:
        ...
        try:
            ...
            if existing is None:
                ...
                try:
                    with db._lock:
                        conn = db._conn
                        if conn is None:
                            raise RuntimeError("ACP session DB connection unavailable")
                        conn.execute(
            ...
            for msg in state.history:
                ...
                if db_content is None:
                    db_content = ""
                elif not isinstance(db_content, str):
                    db_content = json.dumps(db_content, ensure_ascii=False)

                db_tool_name = msg.get("tool_name") or msg.get("name")
                db_tool_call_id = msg.get("tool_call_id")
                db.append_message(
                    session_id=state.session_id,
                    role=msg.get("role", "user"),
                    content=db_content,
                    tool_name=str(db_tool_name) if db_tool_name is not None else "",
                    tool_calls=msg.get("tool_calls"),
                    tool_call_id=str(db_tool_call_id) if db_tool_call_id is not None else "",
                )
        except Exception:
    ...
    def _make_agent(
        ...
        *,
        ...
        setattr(agent, "_print_fn", _acp_stderr_print)
        return agent
