...
from domain.cron.models import ButlerCronJob, ButlerCronRun, CronDeliveryMode
...
async def list_jobs(
    account_id: str = "demo", status: str | None = None, db: AsyncSession = Depends(get_db)
) -> dict:
...
async def get_job(
    job_id: str, account_id: str = "demo", db: AsyncSession = Depends(get_db)
) -> dict:
...
async def job_history(
    job_id: str, account_id: str = "demo", limit: int = 50, db: AsyncSession = Depends(get_db)
) -> dict:
    uid = uuid.UUID(job_id)
    job = await db.get(ButlerCronJob, uid)
    if job is None or job.account_id != account_id:
    ...
    query = (
        select(ButlerCronRun)
        .where(ButlerCronRun.job_id == uid)
        .order_by(ButlerCronRun.triggered_at.desc())
        .limit(limit)
    )
