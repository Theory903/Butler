...
class TestWorkflowEngineDurable:
    ...
    async def test_linear_stepping_log_events(self):
        # 1. Setup a simple 3-node DAG with terminal SUCCESS node
        dag = WorkflowDAG(
            nodes=[
                DAGNode(id="node_1", kind=NodeKind.TASK, tool_name="tool_a", next="node_2"),
                DAGNode(id="node_2", kind=NodeKind.TASK, tool_name="tool_b", next="node_3"),
                DAGNode(id="node_3", kind=NodeKind.SUCCESS, next=None),
            ],
            start_at="node_1",
        )

        workflow = MockWorkflow(id=uuid.uuid4(), plan_schema=dag.model_dump(), status="active")
