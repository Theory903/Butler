...
def mock_ml_runtime():
    runtime = AsyncMock()
    runtime.execute_inference = AsyncMock(return_value={
...
def mock_redis():
    ...
    redis.get = AsyncMock(side_effect=lambda k: json.dumps(session_data) if "butler:session:test_session" in k else None)
...
async def test_orchestrator_triggers_compression():
    ...
    store.get_context = AsyncMock(return_value=ContextPack(
