...
def graph_repo():
    repo = MagicMock()
    repo.get_graph_context = AsyncMock(
        return_value={"interests": ["concise", "tech"], "loyalty": 0.9}
    )
    return repo
...
def personalization_engine(feature_service):
    config = PersonalizationConfig(candidate_limit=10, light_ranker_cutoff=5, heavy_ranker_cutoff=2)
    vs = MagicMock()
    gs = MagicMock()
...
class TestFlight3Personalization:
    ...
    async def test_hybrid_ranking_logic(self, personalization_engine):
        ...
        personalization_engine.feature_hydrator.feature_store.get_features = AsyncMock(
            return_value={
        ...
        personalization_engine.light_ranker.features.get_online_features = AsyncMock(
            return_value=FeatureVector(
        ...
        ranked = await personalization_engine.rank(
            query_vector=np.zeros(128), context=context, candidates=candidates
        )
    ...
    async def test_retrieval_fusion_integration(self, personalization_engine):
        ...
        with patch.object(engine, "_get_candidates", new=AsyncMock(return_value=[m1, m2])):
            ...
            personalization_engine.light_ranker.features.get_online_features = AsyncMock(
                return_value=MagicMock(features={})
            )
