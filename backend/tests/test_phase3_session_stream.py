...
from services.gateway.session_manager import ButlerSessionManager, ButlerSession
...
class TestStreamBridgeSSE:
    ...
    def test_as_sse_sequence(self):
        bridge = self._make_bridge()
        events = asyncio.run(self._collect_sse(bridge, _make_stream(_token_event("Hi"), _final_event())))
